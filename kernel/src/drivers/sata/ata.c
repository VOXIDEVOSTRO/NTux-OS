...
static bool ata_poll(ata_device_t* dev, bool check_error) {
    ...
    status = inb(dev->io_base + ATA_REG_STATUS);
    if (status & ATA_SR_ERR || status & ATA_SR_DF) return false;
    if (check_error && (status & ATA_SR_DRQ) == 0) return false;
...
static bool ata_identify(ata_device_t* dev) {
    ...
    if (!ata_poll(dev, false)) return false;

    uint8_t cl = inb(bus + ATA_REG_LBA1);
    uint8_t ch = inb(bus + ATA_REG_LBA2);
    if (cl != 0 || ch != 0) return false;

    if (!ata_poll(dev, true)) return false;
    ...
    dev->lba48_supported = (dev->identify.capabilities[83] & (1 << 10)) != 0;
...
bool ata_get_disk_ops(DiskOps* ops) {
    if (!g_ata0.present || !ops) return false;
